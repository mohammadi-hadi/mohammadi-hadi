# Hi, I'm Hadi Mohammadi 👋

**AI & Data Science Expert** • **PhD Researcher in Explainable NLP**

I work at **AcademicTransfer** building AI systems and conduct research at **Utrecht University** on making AI more interpretable and culturally aware.

---

## 📊 GitHub Stats

<div align="center">

![Stats](https://github-readme-stats.vercel.app/api?username=mohammadi-hadi&show_icons=true&theme=transparent&hide_border=true&include_all_commits=true&count_private=true&hide=contribs)

![Streak](https://streak-stats.demolab.com/?user=mohammadi-hadi&theme=transparent&hide_border=true)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=mohammadi-hadi&layout=compact&theme=transparent&hide_border=true&langs_count=6)

</div>

---

## 🔬 Research & Work

**PhD Research (Utrecht University)**
- Explainable NLP with cultural awareness
- Moral reasoning in Large Language Models
- Annotation quality and reliability frameworks

**Industry (AcademicTransfer)**
- ML-powered CV ranking and matching systems
- NLP-driven job description optimization
- AI automation tools for academic recruitment

---

## 💻 Tech Stack

**Languages**: Python, R, SQL
**ML/NLP**: PyTorch, Transformers, Hugging Face, BERT, GPT
**Data**: Pandas, NumPy, Scikit-learn
**Tools**: Git, Docker, AWS, Google Cloud

---

## 🚀 Featured Projects

- [**Cultural Moral Judgments with LLMs**](https://github.com/mohammadi-hadi/cultural-moral-judgments-llms) — Cross-cultural AI moral reasoning research
- [**Explainable Annotations Reliability**](https://github.com/mohammadi-hadi/Explainable_Annotations_Reliability) — Annotation quality evaluation framework
- [**AT-CV-Priority-Sorter**](https://github.com/mohammadi-hadi/AT-CV-Priority-Sorter) — ML-powered CV screening system
- [**FBB Sustainability Analysis**](https://github.com/Firmbackbone/fbb-sustainability-analysis-cli) — Environmental impact tracking tool

---

## 📫 Connect

[![Website](https://img.shields.io/badge/Website-mohammadi--hadi.github.io-blue?style=flat-square)](https://mohammadi-hadi.github.io/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0077B5?style=flat-square)](https://www.linkedin.com/in/mohammadi-hadi/)
[![Utrecht University](https://img.shields.io/badge/Utrecht_University-Staff-FFCD00?style=flat-square)](https://www.uu.nl/staff/HMohammadi)

---

<div align="center">
<sub>📊 Stats include private contributions • Most work in private repos at AcademicTransfer</sub>
</div>
