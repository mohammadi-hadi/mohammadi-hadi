...
- [arenakit](https://github.com/mohammadi-hadi/arenakit) — audit a pairwise model leaderboard before you trust its order: Bradley–Terry fitted by MM with tie models and a first-position term, bootstraps clustered on voters or prompts, and a max-t test that controls every adjacent comparison on the board at once instead of judging each rank in isolation — plus an audit of 106,134 real LMArena votes where only 1 of 54 neighbouring pairs survives simultaneous control (5 judged one at a time), and of MT-Bench where GPT-4 reverses its own verdict on 15.8% of pairs when the two answers swap places yet still reproduces the human ranking exactly.
...
- [calikit](https://github.com/mohammadi-hadi/calikit) — audit whether a model's probabilities mean anything: reliability diagrams, calibration error with bootstrap intervals, Brier score with its Murphy decomposition, and Spiegelhalter's test for whether the miscalibration is real rather than sampling noise, plus temperature, Platt and isotonic recalibration — on the bundled example a decisively miscalibrated classifier (Z = 9.2) is repaired by a single temperature fit that leaves no detectable miscalibration behind (T = 2.06, calibration error 0.129 to 0.043). No dependencies; on [PyPI](https://pypi.org/project/calikit/), with an [in-browser explorer](https://mohammadi.cv/calikit/).
- [abeval](https://github.com/mohammadi-hadi/abeval) — treat an eval like an A/B test: confidence intervals for a single run, sign-flip permutation, paired t and exact McNemar tests for two runs scored on the same items, judge-noise reliability, and sample-size planning done before you spend on inference — which makes the usual mistake concrete, since detecting a 3-point gain on a 75% baseline needs 1,568 paired items and a 200-item eval cannot resolve anything under 8.4%. No dependencies; on [PyPI](https://pypi.org/project/abeval/), with an [interactive calculator](https://mohammadi.cv/abeval/).
- [rankkit](https://github.com/mohammadi-hadi/rankkit) — ranking evaluation with error bars, and click metrics that survive position bias: NDCG, MRR, MAP, recall and ERR per query with bootstrap intervals, a paired permutation test for two rankers, and inverse propensity scoring for logs where a click measures relevance times attention rather than relevance — on the bundled example, counting clicks rejects a candidate that is genuinely better by 3.6 points of NDCG, correcting for position reverses the verdict, and the sensitivity sweep then shows the reversal only survives above eta = 0.94, close enough to the assumed 1.0 to send the decision to an online test. No dependencies; with an [in-browser explorer](https://mohammadi.cv/rankkit/).
...
- [judgewatch](https://github.com/mohammadi-hadi/judgewatch) — a public monthly leaderboard of LLM-judge bias: one frozen probe set for position, verbosity, bandwagon and consistency effects, re-run against hosted judge models every month and published with month-over-month deltas, so that a judge drifting between releases becomes visible instead of anecdotal ([leaderboard](https://mohammadi.cv/judgewatch/)).
...
- [explainkit](https://github.com/mohammadi-hadi/explainkit) — explain a text classifier, then make the explanation prove itself: word-level attributions for any `predict_proba` callable, token selection, LLM prompt steering, replacement probes and faithfulness metrics, scored on a corpus whose true rationales are known by construction — where occlusion recovers those rationales at F1 0.77 and flips every prediction it claims to explain, and a permuted-scores baseline reaches only 0.20.
